...
class myBattery(threading.Thread): 
    def __init__(self, name):
        threading.Thread.__init__(self) 
        self.name = name 
        self.count = 0
        self.ServerIp = batteryIp
        # HOST = '1.234.23.229'
        self.ServerPort = batteryPort       
    ...
    def callCreateDb(self, data):
        print('&&&&&&&&&&&&&&&&& in callCreateDb: {}'.format(data))
        ...
        txData = 'http://127.0.0.1:5100/test/{}/{}'.format('battery', data)
    ...
    def serverMain(self):
        ...
        try:
            if data:
                print('@@@@@@@@@@@@@@ receivedData: {}'.format(data.decode()))
...
def main():
    print('now start battery test server. 2022.12.04 12:10')
